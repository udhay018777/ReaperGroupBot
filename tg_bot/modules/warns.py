...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    if is_user_admin(chat, user.id):
        message.reply_text("Damn Admins, can't even be warned. If I could restrict admins then I would soon take over the group!")
        return ""
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "{} warnings, {} has been banned, I won`t welcome him if he enters again!".format(limit, mention_html(user.id, user.first_name))
