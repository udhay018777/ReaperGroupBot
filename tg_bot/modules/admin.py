...
def promote(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id == bot.id:
        message.reply_text(tld(chat.id, "I can't promote myself! Get an admin to do it for me else I will kill you..!"))
        return ""
...
def demote(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        message.reply_text(tld(chat.id, f"Successfully demoted this gey in *{chatD.title}*!"), parse_mode=ParseMode.MARKDOWN)
        return f"<b>{html.escape(chatD.title)}:</b>" \
                "\n#DEMOTED" \
               f"\n<b>Admin:</b> {mention_html(user.id, user.first_name)}" \
               f"\n<b>User:</b> {mention_html(user_member.user.id, user_member.user.first_name)}"
